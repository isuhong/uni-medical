...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    select price into v_price from products where sku = v_item->>'sku';
    if v_price is null then
      raise exception '알 수 없는 SKU: %', v_item->>'sku';
    end if;

    insert into order_items (order_id, sku, qty, price)
    values (v_order_id, v_item->>'sku', (v_item->>'qty')::int, v_price);
    ...
    update inventory
     ...
     where account_id = p_account_id and sku = v_item->>'sku';
  end loop;
